  ...
  try {
    ...
    if (route === "subscribe") {
      ...
      if (!supabase) {
        return res.status(500).json({
          error:
            "Newsletter storage isn't configured yet. Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY.",
        });
      }
      ...
      const tokenToUse =
        existing?.unsubscribe_token || unsubscribeToken;
      ...
      const unsubscribeUrl =
        `${appUrl}/api/newsletter/unsubscribe?email=` +
        `${encodeURIComponent(cleanEmail)}&token=${tokenToUse}`;
    ...
    if (route === "unsubscribe") {
      ...
      if (!supabase) {
        return res
          .status(500)
          .send("Unsubscribe storage isn't configured.");
      }
      ...
      if (error || !data) {
        return res
          .status(400)
          .send("Invalid or expired unsubscribe link.");
      }
    ...
    if (route === "send-welcome") {
      ...
      let unsubscribeUrl =
        `${appUrl}/api/newsletter/unsubscribe?email=` +
        `${encodeURIComponent(cleanEmail)}&token=preview`;
      ...
      return res.json({
        message: sendResult.sent
          ...
          : `Could not send email (${sendResult.reason}).`,
        welcomeEmail: {
          subject: welcomeEmail.subject,
          html: welcomeEmail.html,
          recipient: cleanEmail,
    ...
    if (route === "send-log") {
      ...
      if (!supabase) {
        return res
          .status(500)
          .json({ error: "Storage isn't configured." });
      }
      ...
      if (error) {
        return res
          .status(500)
          .json({ error: "Failed to load sent log." });
      }
    ...
    // ============================================================
    // POST /api/newsletter/generate-weekly-email
    // ============================================================
    if (route === "generate-weekly-email") {
      if (req.method !== "POST") {
        return res.status(405).json({ error: "Method not allowed" });
      }
      ...
      const email = await generateWeeklyEmail({
        recipeTitle,
        season,
        ctaUrl,
      });
    ...
    // ============================================================
    // /api/newsletter/dispatch-weekly
    // Vercel Cron + manual testing
    // ============================================================
    if (route === "dispatch-weekly") {
      ...
      const { forceRegenerate } = (req.body || {}) as {
      ...
      if (error) {
        return res
          .status(500)
          .json({ error: "Failed to load subscribers." });
      }
    ...
    return res.status(404).json({
      error: `Unknown newsletter route: ${route}`,
    });
  } catch (err) {
    console.error(`Error in newsletter route "${route}":`, err);
